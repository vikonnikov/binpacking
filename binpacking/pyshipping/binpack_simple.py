...
from plot import Strip, Layer, Bin
...
def packstrip(bin, p):
    ...
    s = []                # packages in strip
    r = []                # the rest packages, that didn't fit in strip
...
def packlayer(bin, packages):
    ...
    while packages:
        strip, (sizex, stripsize, sizez), rest, dstrip = packstrip(bin, packages)
...
def packit(bin, originalpackages):
    ...
    while packages:
        packagesinbin, (binx, biny, binz), rest, dbin = packbin(bin, packages)
        ...
        dbin.draw()
        dbin.save('bin-%s.png' % len(packedbins))
