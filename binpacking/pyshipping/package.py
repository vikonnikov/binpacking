...
from plot import Cube


class Package(object, Cube):
    ...
    def __init__(self, size, weight=0, nosort=False):
        ...
        self.h, self.w, self.d = self.heigth, self.width, self.length
    ...
    def __repr__(self):
        if self.weight:
            return "< %dx%dx%d %d>" % (self.heigth, self.width, self.length, self.weight)
        else:
            return "%dx%dx%d (%s, %s, %s)" % (self.heigth, self.width, self.length, self.x, self.y, self.z)
